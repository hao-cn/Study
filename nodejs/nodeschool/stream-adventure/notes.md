...
`entry.type` is the kind of file ('File', 'Directory', etc) `entry.path` is the file path
...
Make sure to `npm install tar through` in the directory where your solution file lives.